...
import pandas as pd
# numpy-financial is only used for IRR. It pins older numpy versions and is a
# common source of dependency-resolution failures on hosted environments, so it
# is optional here with an equivalent bisection fallback.
try:
...
def _irr_bisection(cash_flows, lo=-0.9999, hi=10.0, tol=1e-7, max_iter=300):
    """Internal rate of return by bisection. Returns nan if no sign change."""
    def npv(rate):
...
def compute_irr(cash_flows):
    """IRR via numpy-financial when present, otherwise bisection."""
    if NPF_AVAILABLE:
        try:
...
# ── LATITUDE-AWARE AREA MODEL ──────────────────────────────
# Inter-row spacing on a fixed-tilt array is set by shadow length at low winter
# sun, which is a function of latitude. A site at 10°N packs far more capacity
# into the same land than one at 55°N.
#
# GCR below is NET ground coverage ratio — module area divided by TOTAL land
# area — so it already absorbs access roads, setbacks, inverter stations and
# boundary clearances.
#
# Validated against two published airport solar projects 43 degrees of latitude
# apart, both within 4.5%:
#
#   Cochin (CIAL), India   10.15N   12 MWp on 182,109 m2, 157 W/m2 modules
#                                   actual 12.0 MW, model 11.5 MW  (-4.4%)
#   Edmonton (YEG), Canada 53.31N   112 MW on 2,537,370 m2, 190 W/m2 modules
#                                   actual 112.0 MW, model 113.7 MW (+1.5%)
#
# Implied real GCR: Cochin 0.418 vs model 0.401; Edmonton 0.260 vs model 0.236.
# The model runs slightly conservative at both ends, which is the right bias
# for pre-feasibility work.

def gcr_ground(latitude_deg: float) -> float:
    """Net ground coverage ratio for a fixed-tilt ground-mounted array."""
    a = abs(latitude_deg)
    return max(0.15, min(0.50, 0.440 - 0.00383 * a))


def area_per_mw(latitude_deg: float, w_per_m2: float, install_kind: str) -> float:
    """Square metres of site area required per MWp, by latitude and mounting type."""
    g = gcr_ground(latitude_deg)
    if install_kind == "tracking":
        g *= 0.72            # single-axis needs wider rows to avoid self-shading
    elif install_kind == "roof_flat":
        g = min(0.65, g * 1.50)   # lower tilt on a flat roof allows tighter rows
    elif install_kind == "roof_pitched":
        g = 0.68             # flush-mounted, follows the roof — latitude-independent
    elif install_kind == "mixed":
        g *= 1.10
    module_area_per_mw = 1_000_000.0 / max(w_per_m2, 1.0)
    return module_area_per_mw / g
...
with st.sidebar:
    ...
    site_latitude  = st.number_input("Site latitude (°)", -66.0, 66.0, 50.0379, 0.0001,
                                     format="%.4f",
                                     help="Drives inter-row spacing and therefore land area "
                                          "per MW. Also centres the Gate 7 map. "
                                          "Frankfurt 50.04 · Doha 25.27 · Kochi 10.15")
    ...
    module_w_m2    = st.slider("Module power density (W/m²)", 120, 250, 215, 5,
                                help="Modern mono-PERC ≈ 215 W/m². 2015-era modules ≈ 157 W/m². "
                                     "Higher density means less land per MW.")
...
with tabs[0]:
    ...
    with c2:
        built_pct          = st.slider("Built / operational area (%)", 1, 100, 40, 1,
                                        help="Share of the total site occupied by terminals, "
                                             "stands, taxiways, aprons, roads and car parks")
...
with tabs[1]:
    ...
    # Deduction step scales with site size so small sites remain adjustable
    _g2_max  = float(max(gate1_available_m2, 1.0))
    _g2_step = float(max(round(_g2_max / 200, -2) or 100, 100))

    c1, c2 = st.columns(2)
    with c1:
        glare_study_done  = st.checkbox("Formal glare study completed")
        glare_zones_m2    = st.number_input(
            "Area eliminated by glare constraints (m²)", 0.0, _g2_max,
            0.0, _g2_step, format="%0.0f",
        ...
        ols_eliminated_m2 = st.number_input(
            "Area eliminated by OLS height restrictions (m²)", 0.0, _g2_max,
            0.0, _g2_step, format="%0.0f",
            disabled=not ols_study_done,
            help="Obstacle limitation surface height caps")
    with c2:
        rpz_eliminated_m2  = st.number_input(
            "Area within runway protection zones (m²)", 0.0, _g2_max,
            0.0, _g2_step, format="%0.0f",
    ...
    # Flags
    g2_flags = []
    # Fully-constrained site is the headline result — flag it before anything else
    if gate1_available_m2 > 0 and gate2_cleared_m2 <= 0:
        g2_flags.append((
            "fail",
            f"**No developable area remains.** Aviation constraints total "
            f"{g2_eliminated:,.0f} m², which equals or exceeds the "
            f"{gate1_available_m2:,.0f} m² available at Gate 1. Solar is not viable on this "
            f"site as currently constrained — revisit the layout with the aviation authority, "
            f"or reassess whether the full glare and OLS exclusions apply to ground-mounted "
            f"panels at low height."
        ))
    elif gate1_available_m2 > 0 and gate2_cleared_m2 / gate1_available_m2 < 0.10:
        g2_flags.append((
            "warn",
            f"Only {gate2_cleared_m2/gate1_available_m2:.0%} of the Gate 1 area survives "
            f"aviation constraints. Confirm the exclusion zones are correct before "
            f"proceeding — this is a heavily constrained site."
        ))
    ...
    if rpz_eliminated_m2 > 0:
        g2_flags.append(("info", f"{rpz_eliminated_m2:,.0f} m² excluded as runway protection zone — absolute exclusion, correctly deducted"))
    ...
    if gate2_cleared_m2 > 0 and not any(f[0] == "fail" for f in g2_flags):
...
with tabs[5]:
    ...
    with c1:
        ...
        # Area per MW is derived from site latitude and module power density rather
        # than fixed, because inter-row spacing depends on the winter sun angle.
        _kind_map = {
            "Ground-mount – fixed tilt":            "ground",
            "Ground-mount – single-axis tracking":  "tracking",
            "Rooftop – flat":                       "roof_flat",
            "Rooftop – pitched / optimised":        "roof_pitched",
            "Mixed (ground-mount + rooftop)":       "mixed",
        }
        install_kind       = _kind_map[install_type]
        site_gcr           = gcr_ground(site_latitude)
        m2_per_mw_lookup   = {k: area_per_mw(site_latitude, module_w_m2, v)
                              for k, v in _kind_map.items()}
        m2_per_mw          = m2_per_mw_lookup[install_type] * terrain_area_mult
        usable_fraction    = st.slider("Developable fraction of cleared area (%)", 40, 100, 95, 1,
                                        help="Share of the cleared area that is actually buildable — "
                                             "accounts for awkward site geometry, unusable corners and "
                                             "internal obstructions. Do NOT deduct access roads, "
                                             "setbacks or inverter stations here: those are already "
                                             "inside the ground coverage ratio. Use 95–100% for a "
                                             "clean rectangular parcel.") / 100
    with c2:
    ...
    with c3:
        st.metric("Derived area per MW", f"{m2_per_mw:,.0f} m²",
                  help=f"Derived from site latitude {site_latitude:.2f}° and module density "
                       f"{module_w_m2} W/m². Net ground coverage ratio {site_gcr:.3f}. "
                       f"Set latitude in the sidebar.")
    ...
    # IRR
    irr_val = compute_irr(cash_flows)
...
| `f_dev` — developable fraction | **{usable_fraction:.0%}** | Site geometry only — roads and setbacks sit inside the GCR |
...
""")
        ...
        st.markdown("###### How area per MW is derived")
        st.latex(r"GCR_{net} = 0.440 - 0.00383 \\times |\\phi|")
        st.latex(r"a_{MW} = \\frac{1{,}000{,}000 / W_{m^2}}{GCR_{net} \\times k_{mount}}")
        st.markdown(f"""
Inter-row spacing is set by shadow length at low winter sun, so land per MW is a
function of latitude. GCR here is *net* — module area divided by total land area —
so it already absorbs access roads, setbacks, inverter stations and boundary clearances.

| Term | Value | Note |
|---|---|---|
| `φ` — site latitude | **{site_latitude:.2f}°** | Sidebar input |
| `GCR_net` — ground coverage ratio | **{site_gcr:.3f}** | 0.440 − 0.00383 × {abs(site_latitude):.2f} |
| `W_m²` — module power density | **{module_w_m2} W/m²** | Sidebar input |
| Module area per MW | **{1_000_000/module_w_m2:,.0f} m²** | 1,000,000 ÷ {module_w_m2} |
| `k_mount` — mounting factor | see below | Tracking 0.72 · Flat roof ×1.50 · Pitched fixed 0.68 |
| **`a_MW` — area per MW** | **{m2_per_mw:,.0f} m²** | Including terrain multiplier ×{terrain_area_mult:.2f} |

Validated against two published airport projects 43° of latitude apart:
Cochin (CIAL) at 10.15°N — 12 MWp on 182,109 m², model predicts 11.5 MW (−4.4%);
Edmonton (YEG) at 53.31°N — 112 MW on 2,537,370 m², model predicts 113.7 MW (+1.5%).
""")
...
| Less layout and setbacks | −{layout_loss_m2:,.0f} | **{effective_m2:,.0f}** | {usable_fraction:.0%} developable fraction applied at Gate 6 |
...
""")
    ...
    if not FOLIUM_AVAILABLE:
        ...
        with mc1:
            map_lat = st.number_input("Site latitude", -90.0, 90.0,
                                       float(site_latitude), 0.0001, format="%.4f",
                                       help="Defaults to the sidebar latitude")
        with mc2:
...
with tabs[7]:
    ...
    summary_rows = [
        ...
        ("Developable fraction",            round(usable_fraction * 100, 0),   "%"),
        ("Effective PV area",               round(effective_m2, 0),            "m²"),
        ("Effective PV area (ha)",          round(effective_m2/10_000, 2),     "ha"),
        ("Site latitude",                   round(site_latitude, 4),           "°"),
        ("Module power density",             module_w_m2,                       "W/m²"),
        ("Net ground coverage ratio",        round(site_gcr, 3),                ""),
        ("Area per MW (derived)",            round(m2_per_mw, 0),               "m²/MW"),
    ...
    def build_docx() -> bytes:
        ...
        kv_table([
            ...
            ("Site latitude", f"{site_latitude:.2f}", "°"),
            ("Module power density", f"{module_w_m2}", "W/m²"),
            ("Net ground coverage ratio", f"{site_gcr:.3f}", ""),
            ("Area per MW (derived)", f"{m2_per_mw:,.0f}", "m²/MW"),
        ...
        body(f"Area per MW is derived from site latitude rather than assumed. Inter-row "
             f"spacing depends on shadow length at low winter sun, so a site at "
             f"{site_latitude:.2f}° gives a net ground coverage ratio of {site_gcr:.3f}. "
             f"With modules at {module_w_m2} W/m² this yields {m2_per_mw:,.0f} m² per MW. "
             f"The model is calibrated against Cochin International Airport, where a "
             f"published 12 MWp array on 182,109 m² at 10.15°N implies 15,176 m²/MW "
             f"against a modelled 15,879 m²/MW.")
